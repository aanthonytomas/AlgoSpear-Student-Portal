...
import QuizResultPreview from './QuizResultPreview.vue';
...
export default defineComponent({
  ...
  components: { Swiper, SwiperSlide, QuizResultPreview },
  setup() {
  ...
  data() {
    return {
      ...
      showResults: false // Toggle between quiz taking and results view
    }
  },
  methods: {
    onPrevious() {
    ...
    async startTimer() {
      this.stopTimer = timerStartTimer(async (newTime) => {
        ...
        if (newTime?.seconds == 0) {
          ...
          // Remove the isQuizOngoing query parameter to prevent confirmation dialogs
          this.$router.push({
            path: this.$route.path,
  ...
  if (!this.list[index]['answer_input']) {
    ...
    if (answer_correct == answer_input) {
      ...
      this.list[index]['is_correct'] = 1;
    } else {
      this.list[index]['is_correct'] = 0;
    }
    ...
    if (this.swiper.isEnd) {
      if (this.score >= jlconfig.quiz_limit_passing) {
        this.passed = 1;
      }
      ...
      await this.onSaveExcercises().then(async () => {
        ...
        const resultMessage = this.passed
          ? `Congratulations! You passed the quiz with a score of ${this.score}/${jlconfig.quiz_limit}.`
          : `You failed the quiz with a score of ${this.score}/${jlconfig.quiz_limit}. Better luck next time!`;

        const resultIcon = this.passed ? "success" : "error";

        await Swal.fire({
          title: this.passed ? "Quiz Passed!" : "Quiz Failed",
          text: resultMessage,
          icon: resultIcon,
    ...
    reset: async function () {
      if(this.reset > 0) {
        ...
        if(user) {
          ...
          this.showResults = false; // Make sure we're in quiz taking mode
          this.swiper.slideTo(0);
  ...
  created(){
    this.exercise_duration = jlconfig.quiz_time * 60;
    ...
    this.showResults = false; // Start in quiz taking mode
  },
  beforeUnmount(){
